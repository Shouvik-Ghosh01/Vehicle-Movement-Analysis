...
for image_path in image_paths:
    ...
    # Process the image as before (resize, normalize, etc.)
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf) and (scores[i] <= 1.0)):
            # Process detections as before (draw boxes, labels, etc.)
    ...
    # No need to show images here, comment out display lines
